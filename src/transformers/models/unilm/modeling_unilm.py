...
from ...modeling_utils import PreTrainedModel, PoolerAnswerClass, PoolerEndLogits, PoolerStartLogits
#from ..bert.modeling_bert import ACT2FN, BERT_PRETRAINED_MODEL_ARCHIVE_MAP
#from ..roberta.modeling_roberta import ROBERTA_PRETRAINED_MODEL_ARCHIVE_MAP
#from ..xlm_roberta.modeling_xlm_roberta import XLM_ROBERTA_PRETRAINED_MODEL_ARCHIVE_MAP
#from ..electra.modeling_electra import ELECTRA_PRETRAINED_MODEL_ARCHIVE_MAP
from ...file_utils import WEIGHTS_NAME, add_code_sample_docstrings, add_start_docstrings, add_start_docstrings_to_model_forward
from ...activations import ACT2FN

from .configuration_unilm import UniLMFinetuningConfig
...
class UniLMPreTrainedModel(PreTrainedModel):
    ...
    """
    config_class = UniLMFinetuningConfig
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.attention_head_size = config.expand_qk_head_dim \
            if hasattr(config, 'expand_qk_head_dim') and config.expand_qk_head_dim is not None \
            else int(config.hidden_size / config.num_attention_heads)

        self.query = nn.Linear(
            config.hidden_size, self.attention_head_size * self.num_attention_heads)
        self.key = nn.Linear(
            config.hidden_size, self.attention_head_size * self.num_attention_heads, bias=False)
...
class BertAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, encoder_hidden_states=None, split_lengths=None, rel_pos=None):
        if self.layer_norm_type in ('pre', 'hybrid'):
            _hidden_states = self.LayerNorm(hidden_states)
            if encoder_hidden_states is not None:
                _encoder_hidden_states = self.LayerNorm(encoder_hidden_states)
            else:
                _encoder_hidden_states = None
        else:
            _hidden_states = hidden_states
            _encoder_hidden_states = encoder_hidden_states
        self_outputs = self.self(
            _hidden_states, attention_mask=attention_mask,
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        if self.config.rel_pos_bins > 0:
            ...
            rel_pos_one_hot = F.one_hot(
                rel_pos_ids, num_classes=self.config.rel_pos_bins).type_as(embedding_output)
            ...
            rel_pos_embedding = rel_pos_embedding.expand(
                batch_size, -1, seq_len, -1)
            num_attention_head = rel_pos_embedding.size()[1]
            rel_pos_mat = (
                rel_pos_mat - min_distance).unsqueeze(1).expand(-1, num_attention_head, -1, -1)
            rel_pos = torch.gather(
                rel_pos_embedding, dim=-1, index=rel_pos_mat)
        else:
            rel_pos = None
...
class UniLMForSequenceToSequence(UniLMPreTrainedModel):
    MODEL_NAME = 'basic class'

    def __init__(self, config):
        super(UniLMForSequenceToSequence, self).__init__(config)
...
class BertForSequenceToSequenceWithPseudoMask(UniLMForSequenceToSequence):
...
class BertForSequenceToSequenceUniLMV1(UniLMForSequenceToSequence):
...
class UniLMForSequenceClassification(UniLMPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = UniLMModel(config)
        cls_dropout_prob = config.cls_dropout_prob
        self.dropout = nn.Dropout(cls_dropout_prob)
...
class UniLMForQuestionAnswering(UniLMPreTrainedModel):
    r"""
    ...
    def __init__(self, config):
        super(UnilmForQuestionAnswering, self).__init__(config)
        self.num_labels = config.num_labels
...
class UnilmForQuestionAnsweringPlus(UniLMPreTrainedModel):
    def __init__(self, config):
        super(UnilmForQuestionAnsweringPlus, self).__init__(config)
        self.start_n_top = config.beam_size
        self.end_n_top = config.beam_size
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, inputs_embeds=None,
                start_positions=None, end_positions=None, p_mask=None, cls_index=None, is_impossible=None):
        ...
        if start_positions is not None and end_positions is not None:
            ...
            if cls_index is not None and is_impossible is not None:
                ...
                cls_loss = loss_fct_cls(
                    cls_logits, is_impossible.type_as(cls_logits))
            ...
            start_states = torch.einsum(
                "blh,bl->bh", hidden_states, start_log_probs.exp(),
            )  # get the representation of START as weighted sum of hidden states
            cls_logits = self.answer_class(
                hidden_states, start_states=start_states, cls_index=cls_index
            )  # Shape (batch size,): one single `cls_logits` for each sample

            return (start_top_log_probs, start_top_index, end_top_log_probs, end_top_index, cls_logits) + outputs


class GetLogits(nn.Module):
    """ Compute SQuAD 2.0 answer class from classification and start tokens hidden states. """

    def __init__(self, input_hidden_size, middle_hidden_size, need_ln, need_dropout, config):
        super().__init__()
        self.input_dense = nn.Linear(input_hidden_size, middle_hidden_size)
        self.activation = nn.Tanh()
        if need_dropout:
            self.dropout = nn.Dropout(
                config.cls_dropout_prob if hasattr(config, 'cls_dropout_prob') else config.hidden_dropout_prob)
        else:
            self.dropout = None
        if need_ln:
            self.layer_norm = BertLayerNorm(
                middle_hidden_size, eps=config.layer_norm_eps)
        else:
            self.layer_norm = None
        self.output_dense = nn.Linear(middle_hidden_size, 1, bias=False)
...
class SQuADTaskLayerForUniLM(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.start_n_top = config.beam_size
        self.end_n_top = config.beam_size
        ...
        self.start = nn.Linear(config.hidden_size, 1, bias=False)
        self.end = GetLogits(
            input_hidden_size=config.hidden_size * 2, middle_hidden_size=self.task_hidden_size,
            need_ln=True, need_dropout=False, config=config,
        )
        self.answer = GetLogits(
            input_hidden_size=config.hidden_size * 2, middle_hidden_size=self.task_hidden_size,
            need_ln=False, need_dropout=True, config=config,
        )

    def forward(self, hidden_states, start_positions=None, end_positions=None,
                p_mask=None, cls_index=None, is_impossible=None):
        def to_mask(x, mask):
            if next(self.parameters()).dtype == torch.float16:
                return x * (1 - mask) - 10000.0 * mask
            else:
                return x * (1 - mask) - 1e30 * mask

        start_logits = self.start(hidden_states).squeeze(-1)
        if p_mask is not None:
            start_logits = to_mask(start_logits, p_mask)

        batch_size, sequence_lengths, hidden_size = hidden_states.size()

        if start_positions is not None and end_positions is not None:
            # start_positions -> [Bsz, 1, H]
            start_hidden_states = \
                hidden_states.gather(
                    index=start_positions.view(-1, 1, 1).expand(-1, -1, hidden_size), dim=1)
            start_hidden_states = start_hidden_states.expand(
                -1, sequence_lengths, -1)
            final_repr = torch.cat(
                (start_hidden_states, hidden_states), dim=-1)
            end_logits = self.end(final_repr)
            if p_mask is not None:
                end_logits = to_mask(end_logits, mask=p_mask)
            ...
            if cls_index is not None and is_impossible is not None:
                # Predict answerability from the representation of CLS and START
                start_states = torch.einsum(
                    "blh,bl->bh", hidden_states, F.softmax(
                        start_logits, dim=-1),
                )
                cls_states = hidden_states.gather(
                    index=cls_index.view(-1, 1, 1).expand(-1, -1, hidden_size), dim=1).squeeze(1)
                answer_repr = torch.cat((start_states, cls_states), dim=-1)
                cls_logits = self.answer(answer_repr)
            ...
            # shape (bsz, start_n_top, hsz)
            start_top_index_exp = start_top_index.unsqueeze(
                -1).expand(-1, -1, hidden_size)
            start_hidden_states = torch.gather(
                hidden_states, -2, start_top_index_exp)  # shape (bsz, start_n_top, hsz)
            start_hidden_states = start_hidden_states.unsqueeze(
                1).expand(-1, sequence_lengths, -1, -1)  # shape (bsz, slen, start_n_top, hsz)
            ...
            p_mask = p_mask.unsqueeze(-1) if p_mask is not None else None
            final_repr = torch.cat(
                (start_hidden_states, hidden_states_expanded), dim=-1)
            end_logits = self.end(final_repr)
            if p_mask is not None:
                end_logits = to_mask(end_logits, mask=p_mask)
            ...
            cls_states = hidden_states.gather(index=cls_index.view(-1, 1, 1).expand(-1, -1, hidden_size),
                                              dim=1).squeeze(1)
            answer_repr = torch.cat((start_states, cls_states), dim=-1)
            cls_logits = self.answer(answer_repr)
...
class UniLMForQuestionAnsweringPro(UniLMPreTrainedModel):
    def __init__(self, config):
        super(UnilmForQuestionAnsweringPro, self).__init__(config)

        self.config = config
        self.bert = UniLMModel(config)
        self.task_layer = SQuADTaskLayerForUniLM(config)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, inputs_embeds=None,
        ...
        return self.task_layer(
            hidden_states=hidden_states, start_positions=start_positions, end_positions=end_positions,
            p_mask=p_mask, cls_index=cls_index, is_impossible=is_impossible) + outputs
