...
from ...modeling_utils import cached_path, WEIGHTS_NAME, TF2_WEIGHTS_NAME, TF_WEIGHTS_NAME
...
def unilm_to_hf_bert(state_dict):
    logger.info(" * Convert checkpoint format to Huggingface BERT format * ")

    # checkpoint dumped from deepspeed
    if 'module' in state_dict:
        state_dict = state_dict['module']

    new_state_dict = {}

    if 'bert.encoder.rel_pos_bias_ghost' in state_dict:
        rp = state_dict['bert.encoder.rel_pos_bias.weight'].clone().float()
        rp_ghost = state_dict['bert.encoder.rel_pos_bias_ghost'].clone().float()
        state_dict['bert.encoder.rel_pos_bias.weight'] = (
                rp + rp_ghost).type_as(state_dict['bert.encoder.rel_pos_bias.weight'])
        del state_dict['bert.encoder.rel_pos_bias_ghost']
