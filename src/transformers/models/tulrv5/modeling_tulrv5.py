...
from ..bert.modeling_bert import BertIntermediate, BertOutput, BertPooler, BertSelfOutput, BertEmbeddings

from .configuration_tulrv5 import TULRv5Config
...
TULRv5_PRETRAINED_MODEL_ARCHIVE_MAP = {
    'tulrv5-base': "https://turingdatashare.blob.core.windows.net/projectalexander/languagemodels/tulrv5/base/pytorch_model.bin",
    'tulrv5-large': "https://turingdatashare.blob.core.windows.net/projectalexander/languagemodels/tulrv5/large/pytorch_model.bin",
}
...
class TULRv5Embeddings(BertEmbeddings):
    """
...
class TULRv5Attention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.self(
            hidden_states, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask, rel_pos=rel_pos
        )
...
class TULRv5Layer(nn.Module):
    def __init__(self, config, layer_idx=-1):
        ...
        if self.is_decoder:
            self.crossattention = TULRv5Attention(config)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask, rel_pos=rel_pos
            )
...
class TULRv5PreTrainedModel(PreTrainedModel):
    ...
    pretrained_model_archive_map = TULRv5_PRETRAINED_MODEL_ARCHIVE_MAP
...
class TULRv5Model(TULRv5PreTrainedModel):
    def __init__(self, config):
        ...
        self.embeddings = TULRv5Embeddings(config)
    ...
    Examples::
        ...
        extended_attention_mask = extended_attention_mask.to(dtype=attention_mask.dtype, device=device)  # fp16 compatibility
...
class TULRv5ForQuestionAnswering(TULRv5PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
