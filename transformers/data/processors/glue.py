...
def glue_convert_examples_to_features(examples, tokenizer,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if ex_index < 5:
            ...
            logger.info("text_c: %s" % (example.text_c))
        ...
        features.append(
                PairInputFeatures(guids=example.guid,
    ...
    if is_tf_available() and is_tf_dataset:
        def gen():
            for ex in features:
                yield  ({'guids': ex.guids,
                         'input_ids_a': ex.input_ids_a,
                         'input_ids_b': ex.input_ids_b,
                         'attention_mask_a': ex.attention_mask_a,
                         'attention_mask_b': ex.attention_mask_b,
                         'token_type_ids_a': ex.token_type_ids_a,
                         'token_type_ids_b': ex.token_type_ids_b
                         },
                        ex.label)
        ...
        return tf.data.Dataset.from_generator(gen,
            ({'guids': tf.int32,
              ...
              'token_type_ids_a': tf.int32,
              'token_type_ids_b': tf.int32
              },
             tf.int64),
            ({'guids': tf.TensorShape([None]),
              ...
              'token_type_ids_a': tf.TensorShape([None]),
              'token_type_ids_b': tf.TensorShape([None])
              },
             tf.TensorShape([])))
...
class PairProcessor(DataProcessor):
    ...
    def get_example_from_tensor_dict(self, tensor_dict):
        ...
        return InputExample(tensor_dict['idx'].numpy(),
                            ...
                            tensor_dict['Passage1'].numpy().decode('utf-8'),
                            tensor_dict['Passage2'].numpy().decode('utf-8'))
